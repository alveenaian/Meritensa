    ...
    },
    {
      ...
      plugins: () => [
        config("allowedHosts", {
          // @ts-ignore
          server: {
            // In development or preview environments, allow all hosts
            // In production with BASE_URL set, restrict to that host
            allowedHosts: env.NODE_ENV === "development" || !env.BASE_URL 
              ? true 
              : [
                  env.BASE_URL.split("://")[1]?.split(":")[0],
                  // Also allow preview subdomains for TrySolid/Codapt
                  ".codapt.app",
                  ".trysolid.com"
                ].filter(Boolean),
          },
